...
class CalendarTVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    // Source of data
    var calendars: CalbiticaCalendars?
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return (calendars != nil) ? calendars!.data.count : 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cal = calendars!.data[indexPath.row] {
            cell.textLabel?.text = cal.summary
            cell.textLabel?.textColor = cal.sync
                ? CalbiticaColors.blue : .white
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if var cal = calendars!.data[indexPath.row] {
